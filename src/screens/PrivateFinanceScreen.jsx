...
export default function FinanceScreen() {
  ...
  const [subTab,    setSubTab]    = useState('transactions') // 'transactions' | 'tracker' | 'sobras'
  ...
  return (
    <div className="screen">
      {/* Add button only on transactions tab */}
      {subTab === 'transactions' && (
      ...
      <div style={{ display:'flex', gap:0, margin:'10px 14px 0', background:'#f0efe8', borderRadius:10, padding:3, flexShrink:0 }}>
        {[{v:'transactions',l:'💰 Transações'},{v:'tracker',l:'📋 Controle'},{v:'sobras',l:'💸 Sobras'}].map(x=>(
          <button key={x.v} onClick={()=>setSubTab(x.v)} style={{
            flex:1, padding:'8px 4px', border:'none', borderRadius:8, cursor:'pointer', fontSize:11, fontWeight:500,
      ...
      {subTab === 'transactions' && (
        <div className="screen-scroll">
          ...
          {[...transactions].sort((a,b)=>{ const dc=a.date.localeCompare(b.date); return dc!==0?dc:(a.desc||'').localeCompare(b.desc||'') }).map(t=>(
      ...
      {subTab === 'tracker' && (
        <div className="screen-scroll">
          ...
          {/* Add new row AT TOP */}
          <div style={{ background:'#EEEDFE', borderRadius:12, border:'0.5px solid #534AB722', padding:'12px', marginBottom:10 }}>
            <div style={{ fontSize:11, color:'#534AB7', fontWeight:600, marginBottom:8 }}>＋ Nova linha</div>
            ...
            <div style={{ display:'flex', gap:6, alignItems:'center', flexWrap:'wrap' }}>
              <div style={{ fontSize:11, color:'#888' }}>Cor:</div>
              {ROW_COLORS.map(c=>(
          ...
          {/* Rows sorted by date then amount desc */}
          {[...trackerRows].sort((a,b)=>{
            const dc=(a.date||'').localeCompare(b.date||'')
            return dc!==0 ? dc : (parseFloat(b.amount)||0)-(parseFloat(a.amount)||0)
          }).map(row=>(
            <div key={row.id} style={{ display:'grid', gridTemplateColumns:'60px 1fr 90px 32px', gap:0, background:row.color||'#fff', padding:'8px 10px', borderBottom:'0.5px solid #e4e2dc', alignItems:'center' }}>
              {editRowId===row.id ? (
                ...
                <>
                  ...
                  <div style={{ fontSize:13, fontWeight:600, textAlign:'right', textDecoration:row.checked?'line-through':'none', color:row.checked?'#bbb':'#1a1a18' }}>
      ...
      {subTab === 'sobras' && (
        <div className="screen-scroll">
          <div style={{ fontSize:12, color:'#888', marginBottom:12, marginTop:8 }}>
            Controle de sobras mensais · todos os meses do ano
          </div>
          {Array.from({length:12},(_,mi)=>{
            ...
            const save = async (field, val) => {
              if (doc.id) await updateSobra(doc.id, { [field]: val })
              else await addSobra({ monthKey:key, [field]: val })
            }
            const sobrou    = parseFloat(doc.sobrou)    || 0
            const gastos50  = sobrou * 0.5
            const livres30  = sobrou * 0.3
            const invest20  = sobrou * 0.2
            const isOpen    = doc.open !== false
            return (
              <div key={mi} style={{ marginBottom:10, border:'0.5px solid #eee', borderRadius:12, overflow:'hidden' }}>
                {/* Month header */}
                <div onClick={async()=>{ if(doc.id) await updateSobra(doc.id,{open:!isOpen}); else await addSobra({monthKey:key,open:false}) }}
                  style={{ background:'#534AB7', padding:'10px 14px', display:'flex', justifyContent:'space-between', alignItems:'center', cursor:'pointer' }}>
                  <span style={{ fontSize:13, fontWeight:700, color:'#fff' }}>{MONTHS_PT[mi]}</span>
                  <span style={{ color:'#fff9', fontSize:12 }}>{isOpen?'▲':'▼'}</span>
                </div>
                {isOpen && (
                  <div style={{ padding:'10px 0' }}>
                    {/* Sobrou neste mês */}
                    <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:0, padding:'8px 12px', background:'#FCEBEB', alignItems:'center' }}>
                      <span style={{ fontSize:13, fontWeight:600, color:'#791F1F' }}>Sobrou neste mês</span>
                      <input type="number" placeholder="R$ 0" defaultValue={doc.sobrou||''} onBlur={e=>save('sobrou',parseFloat(e.target.value)||0)}
                    ...
                    <div style={{ padding:'8px 12px', background:'#EEEDFE' }}>
                      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:4 }}>
                        <span style={{ fontSize:12, fontWeight:600, color:'#3C3489' }}>20% de investimento</span>
                        <span style={{ fontSize:12, fontWeight:700, color:'#3C3489' }}>{fmtBRL(invest20)}</span>
                      </div>
                      <div style={{ display:'flex', alignItems:'center', gap:6 }}>
                        <span style={{ fontSize:11, color:'#7F77DD' }}>Onde foi:</span>
                        <input type="text" placeholder="Ex: Tesouro Direto, CDB..." defaultValue={doc.investDest||''} onBlur={e=>save('investDest',e.target.value)}
                          style={{ flex:1, fontSize:11, border:'0.5px solid #9B99C4', borderRadius:6, padding:'4px 8px', background:'#fff' }}/>
                      </div>
                    </div>
                  </div>
                )}
