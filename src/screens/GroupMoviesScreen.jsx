...
const GENRES = ['Ação','Aventura','Comédia','Drama','Terror','Sci-Fi','Romance','Comédia romântica','Besteirol','Animação','Documentário','Thriller','Fantasia','Crime','Outros']
const PLATFORMS = ['Netflix','Amazon Prime','Disney+','HBO Max','Apple TV+','Globoplay','Crunchyroll','YouTube','Cinema','Outro']

export default function GroupMoviesScreen() {
  const { items: titles, add, update, remove } = useData('groupMovies','group')
  const [tab,      setTab]      = useState('watchlist')
  const [typeF,    setTypeF]    = useState('all')
  const [showForm, setShowForm] = useState(false)
  const [editItem, setEditItem] = useState(null)
  const [search,   setSearch]   = useState('')
  const emptyForm = { title:'', type:'movie', genre:'Outros', status:'watchlist', platform:'', seasons:'', rating:0, notes:'', coverUrl:'', emoji:'' }
  ...
  return (
    <div className="screen">
      ...
      <div style={{display:'flex',gap:8,padding:'10px 14px 0',flexShrink:0}}>
        {[{id:'watchlist',label:'Quero ver',icon:'🔖',bg:'#FAEEDA',tc:'#633806'},{id:'watching',label:'Assistindo',icon:'▶️',bg:'#EEEDFE',tc:'#534AB7'},{id:'done',label:'Assistidos',icon:'✅',bg:'#EAF3DE',tc:'#27500A'}].map(s=>(
          <div key={s.id} onClick={()=>setTab(s.id)} style={{flex:1,background:s.bg,borderRadius:12,padding:'10px 8px',textAlign:'center',cursor:'pointer',border:tab===s.id?`2px solid ${s.tc}`:'2px solid transparent'}}>
      ...
      <div style={{padding:'8px 14px 4px',display:'flex',gap:6,flexShrink:0}}>
        {[{v:'all',l:'Tudo'},{v:'movie',l:'🎬 Filmes'},{v:'series',l:'📺 Séries'}].map(x=>(
          <button key={x.v} onClick={()=>setTypeF(x.v)} style={{padding:'4px 12px',borderRadius:20,border:'0.5px solid #ddd',background:typeF===x.v?'#534AB7':'#fff',color:typeF===x.v?'#fff':'#888',fontSize:12,cursor:'pointer'}}>{x.l}</button>
        ))}
      </div>
            ...
            <div className="form-group">
              ...
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:0,background:'#f0efe8',borderRadius:10,padding:3}}>
                {[{v:'movie',l:'🎬 Filme'},{v:'series',l:'📺 Série'}].map(x=>(
                  <button key={x.v} onClick={()=>setForm({...form,type:x.v})} style={{padding:'8px',border:'none',borderRadius:8,cursor:'pointer',fontSize:13,background:form.type===x.v?'#fff':'transparent',color:form.type===x.v?'#1a1a18':'#999',boxShadow:form.type===x.v?'0 1px 3px rgba(0,0,0,.1)':'none'}}>{x.l}</button>
                ))}
              </div>
              ...
              {form.type==='series'&&<input type="number" placeholder="Nº de temporadas" value={form.seasons} onChange={e=>setForm({...form,seasons:e.target.value})}/>}
              <div>
                <label className="form-label">Status</label>
                <div style={{display:'flex',gap:4}}>
                  {[{v:'watchlist',l:'Quero ver'},{v:'watching',l:'Assistindo'},{v:'done',l:'Assistido'}].map(s=>(
                    <button key={s.v} onClick={()=>setForm({...form,status:s.v})} style={{flex:1,padding:'7px 4px',borderRadius:8,border:'none',cursor:'pointer',fontSize:11,background:form.status===s.v?'#534AB7':'#f0efe8',color:form.status===s.v?'#fff':'#888'}}>{s.l}</button>
                  ))}
                </div>
              </div>
