...
export default function AnnualScreen() {
  ...
  const catSpend={}
  transactions.filter(t=>{const d=new Date(t.date+'T12:00');return d.getFullYear()===year&&t.type==='out'}).forEach(t=>{catSpend[t.category]=(catSpend[t.category]||0)+t.amount})
    ...
    <div className="screen"><div className="screen-scroll">
      ...
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:8,marginBottom:14}}>
        {[{bg:'#EAF3DE',tc:'#27500A',lc:'#3B6D11',label:'ENTRADAS',val:fmtBRL(totInc)},
          ...
          {bg:totInc-totExp>=0?'#EEEDFE':'#FCEBEB',tc:totInc-totExp>=0?'#3C3489':'#791F1F',lc:'#534AB7',label:'SALDO',val:(totInc-totExp>=0?'+':'-')+fmtBRL(totInc-totExp)}
        ].map(x=>(
          <div key={x.label} style={{background:x.bg,borderRadius:12,padding:'12px'}}>
            <div style={{fontSize:10,color:x.lc,fontWeight:600}}>{x.label}</div>
            <div style={{fontSize:14,fontWeight:700,color:x.tc,marginTop:2}}>{x.val}</div>
          </div>
        ))}
      </div>
      <div className="section-label">Por mês</div>
      <div style={{display:'flex',gap:12,marginBottom:8}}>
        {[{c:'#639922',l:'Entradas'},{c:'#E24B4A',l:'Saídas'}].map(x=>(
          <div key={x.l} style={{display:'flex',alignItems:'center',gap:4}}>
            <div style={{width:8,height:8,borderRadius:'50%',background:x.c}}/><span style={{fontSize:11,color:'#888'}}>{x.l}</span>
          </div>
        ))}
      </div>
      {monthly.map((m,i)=>(
        <div key={i} style={{marginBottom:10,opacity:m.count===0?0.35:1}}>
          <div style={{display:'flex',alignItems:'center',gap:8}}>
            <span style={{width:30,fontSize:12,fontWeight:i===curM?600:400,color:i===curM?'#534AB7':'#888'}}>{MONTHS[i]}</span>
            <div style={{flex:1,display:'flex',flexDirection:'column',gap:3}}>
              <div style={{height:8,background:'#f0efe8',borderRadius:20,overflow:'hidden'}}><div style={{height:'100%',width:`${Math.round(m.inc/maxVal*100)}%`,background:'#639922',borderRadius:20}}/></div>
              <div style={{height:8,background:'#f0efe8',borderRadius:20,overflow:'hidden'}}><div style={{height:'100%',width:`${Math.round(m.exp/maxVal*100)}%`,background:'#E24B4A',borderRadius:20}}/></div>
            </div>
            <span style={{width:70,textAlign:'right',fontSize:12,fontWeight:600,color:m.bal>=0?'#27500A':'#A32D2D'}}>
              {m.count===0?'—':(m.bal>=0?'+':'-')+fmtBRL(m.bal)}
            </span>
          </div>
        </div>
      ))}
      {topCats.length>0&&<><div className="section-label" style={{marginTop:10}}>Maiores gastos</div>
